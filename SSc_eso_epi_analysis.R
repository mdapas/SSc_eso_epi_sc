...
#pdf(file=paste0(output_dir, '/Epi_violin_nCountFilter.pdf'), width=8, height=4)
#ggplot(epi_set@meta.data, aes(x = integrated_snn_res.0.5, y = nCount_RNA, fill=integrated_snn_res.0.5))+
...
# UMAP prior to
epi_set <- RunUMAP(object=epi_set, assay='integrated', dims = 1:35, n.neighbors=150L, min.dist=0.3,
                   spread=1, repulsion.strength=0.9, negative.sample.rate = 10L, n.epochs=500)

#epi_filtered <- subset(epi_set, subset=nCount_RNA>3500)
...
epi_set <- loadRData(paste0(output_dir, '/MP_eso_epi_reintegratedObj.RData'))
#epi_set <- RunPCA(object=epi_set, assay='integrated')


pdf(file=paste0(output_dir, '/epi_PC_plot_integratedAssay.pdf'), width=5, height=6)
ElbowPlot(epi_set, ndims=50)
dev.off()


#epi_set <- RunUMAP(object=epi_set, assay='integrated', dims = 1:35, n.neighbors=100L, min.dist=0.3,
#                   spread=1, repulsion.strength=0.9, negative.sample.rate = 10L, n.epochs=500)
...
#epi_set@reductions$umap@cell.embeddings <- epi_set@reductions$umap@cell.embeddings*-1

pdf(file=paste0(output_dir, '/Epi_UMAP_condition.pdf'), width=7, height=6)
DimPlot(epi_set, reduction='umap', label=F, group.by='condition', shuffle=T, raster=T, pt.size=0.01,
...
pdf(file=paste0(output_dir, '/Epi_UMAP_location.pdf'), width=7, height=6)
DimPlot(epi_set, reduction='umap', label=F, group.by='location', shuffle=T, 
        cols=c('#FF00FF','#00FFFF','#FFFF00')) + ggtitle('') &
  theme(plot.title=element_text(face='bold'  , size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=8, face='bold'), legend.text=element_text(size=8))
...
pdf(file=paste0(output_dir, '/Epi_UMAP_diffBin.pdf'), width=7, height=6)
print(DimPlot(epi_set, reduction='umap', label=F, group.by='diff.bin', shuffle=T, raster=F, pt.size=0.01,
        cols=brewer.pal(n=11,'Spectral')[11:1]) + ggtitle('') &
  theme(plot.title=element_text(face='bold'  , size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=8, face='bold'), legend.text=element_text(size=8)))
dev.off()
...
for (sample in samples) {
  ...
  pdf(file=paste0(output_dir, '/Epi_UMAP_SampleDistributions/Epi_dimPlot_',sample,'.pdf'), width=5, height=5)
  print(DimPlot(epi_set, reduction='umap', label=F, group.by='sampleBin', order=T,
...
epi_set <- CellCycleScoring(epi_set, s.features = s.genes, g2m.features = g2m.genes, set.ident = F)
epi_set$CC.diff <- epi_set$S.Score - epi_set$G2M.Score

pdf(file=paste0(output_dir, '/Epi_UMAP_G2Mscore.pdf'), width=7, height=6)
FeaturePlot(epi_set, features='G2M.Score') &
  theme(plot.title=element_text(face='bold', size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=7, face='bold'), legend.text=element_text(size=6.5))
dev.off()

pdf(file=paste0(output_dir, '/Epi_UMAP_Sscore.pdf'), width=7, height=6)
FeaturePlot(epi_set, features='S.Score') &
  theme(plot.title=element_text(face='bold', size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=7, face='bold'), legend.text=element_text(size=6.5))
dev.off()

pdf(file=paste0(output_dir, '/Epi_UMAP_CCdiff.pdf'), width=7, height=6)
FeaturePlot(epi_set, features='CC.diff') &
  theme(plot.title=element_text(face='bold', size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=7, face='bold'), legend.text=element_text(size=6.5))
dev.off()

pdf(file=paste0(output_dir, '/Epi_UMAP_phase.pdf'), width=7, height=6)
DimPlot(epi_set, reduction='umap', label=F, group.by='Phase', shuffle=T, raster=T, pt.size=0.01,
        cols=c('#85AD51','#59B9BF','#A081B5')) + ggtitle('') &
  theme(plot.title=element_text(face='bold', size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=8, face='bold'), legend.text=element_text(size=8))
dev.off()

pdf(file=paste0(output_dir, '/Epi_violin_nCount.pdf'), width=8, height=5)
ggplot(epi_set@meta.data, aes(x = integrated_snn_res.0.5, y = nCount_RNA, fill=integrated_snn_res.0.5))+
  geom_violin(scale='width') + labs(y= "nCount_RNA", x= "Cluster") + ggtitle('Epithelial Cell Clusters') + 
  theme_classic() + theme(axis.title=element_text(face='bold',size=8), axis.ticks = element_blank(), 
                          legend.position='none', panel.grid.major.y = element_line(), panel.grid.minor.y=element_line()) +
...
#epi_filtered <- subset(epi_set, subset=nCount_RNA>3500)
test_dat <- ScaleData(epi_set, vars.to.regress=c('CC.diff'), assay='integrated')

save(epi_set, file=paste0(output_dir, '/MP_eso_epi_reintegratedObj.RData'))
# Cell typ module heatmaps


module_plots <- list(
  module_plot(epi_set, 'Basal', c('PDPN', 'IGFBP3','COL17A1','KRT15','DST')),
  module_plot(epi_set, 'Suprabasal', c('KRT4','KRT13','SERPINB3','DSC2','IVL')),
  module_plot(epi_set, 'Superficial', c('KRT17','KRT78','FLG','CNFN','CRCT1')))

metallo_genes <- c("MT1A","MT1E","MT1F","MT1G","MT1H","MT1M","MT1X","MT2A")

epi_set <- AddModuleScore(epi_set, features=metallo_genes, name='metal_score', assay='RNA')
  
metal_plots <- lapply(c('Proximal','Distal'), function(loc) {
  lapply(c('HC','SSc'), function(cond) {
    print(c(loc, cond))
    return(FeaturePlot(subset(epi_set, location==loc & condition==cond), features='metal_score1', order=T) & labs(x='UMAP 1', y='UMAP 2') &
             scale_colour_gradientn(colours=brewer.pal(n=11,'Spectral')[11:1], name='Avg. Gene\nExpression\n', 
                                    breaks=c(min(epi_set@meta.data$metal_score1),max(epi_set@meta.data$metal_score1)),
                                    labels=c('Min', 'Max'), guide=guide_colorbar(frame.colour='black', ticks=F)) & 
...
pdf(file=paste0(output_dir, '/Epi_modulePlots_metalloGenes.pdf'), width=8, height=8)
print(plot_grid(plotlist=metal_plots, ncol=2))
dev.off()


# look at superficial markers by condition
library(patchwork)
epi_set$condition <- factor(epi_set$condition, levels=c('HC','GERD','SSc'))
featurePlot <- function(feats, cols) {
  suppressWarnings({
    lapply(c(feats), function(feat) {
      return(FeaturePlot_scCustom(epi_set, features=feat, order=T, split.by='condition', na_cutoff=NA,split_collect = T,
                                  pt.size=1, combine=F,colors_use=cols) & labs(x='UMAP 1', y='UMAP 2') &
               theme(plot.title=element_text(face='bold', size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
                     axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
...
super_marker_plots <- featurePlot(c('CNFN','FLG','IVL'), c('grey80',brewer.pal(n=9,'Reds')[2:9],'black'))

pdf(file=paste0(output_dir, '/epi_featurePlot_CNFN_FLG_IVL.pdf'), width=10, height=9)
patchwork::wrap_plots(super_marker_plots, nrow=3)
dev.off()


epi_set$layers_rep <- gsub(" ", '\n', capitalize(epi_set$layers_rep))
epi_set$layers_rep <- gsub("Replicating", 'Proliferating', capitalize(epi_set$layers_rep))
epi_set$layers_rep <- factor(epi_set$layers_rep, levels=c('Basal','Proliferating\nbasal',
                                                          'Proliferating\nsuprabasal',
                                                          'Suprabasal','Superficial'))
epi_set$condition <- factor(epi_set$condition, levels=c('HC','GERD','SSc'))


pdf(file=paste0(output_dir, '/epi_VlnPlot_termDiff_seuratClusters.pdf'), width=3, height=6)
print(Stacked_VlnPlot(subset(epi_set, seurat_clusters %in% c(6,7,8)), c('CNFN','CRNN','FAM3D','SPRR2D','KRT78','LYPD2','FLG','IVL'), 
        split.by='condition', group.by='seurat_clusters',pt.size=0, colors_use=rev(cond_cols)))
dev.off()

pdf(file=paste0(output_dir, '/epi_VlnPlot_termDiff_diffBin.pdf'), width=3.5, height=6)
print(Stacked_VlnPlot(subset(epi_set, diff.bin %in% c(7:10)), c('CNFN','CRNN','FAM3D','SPRR2D','KRT78','LYPD2','FLG','IVL'), 
                      split.by='condition', group.by='diff.bin',pt.size=0, colors_use=rev(cond_cols)))
dev.off()
...
epi_set <- AddModuleScore(epi_set, features=foxm1_target_genes, name='foxm1_targets', assay='RNA')

foxm1_plots <- lapply(c('Proximal','Distal'), function(loc) {
  lapply(c('HC','SSc'), function(cond) {
    print(c(loc, cond))
    return(FeaturePlot(subset(epi_set, location==loc & condition==cond), features='foxm1_targets1', order=T) & labs(x='UMAP 1', y='UMAP 2') &
             scale_colour_gradientn(colours=brewer.pal(n=11,'Spectral')[11:1], name='Avg. Gene\nExpression\n', 
                                    breaks=c(min(epi_set@meta.data$foxm1_targets1),max(epi_set@meta.data$foxm1_targets1)),
                                    labels=c('Min', 'Max'), guide=guide_colorbar(frame.colour='black', ticks=F)) & 
...
pdf(file=paste0(output_dir, '/Epi_modulePlots_FOXM1_UpGenes.pdf'), width=8, height=8)
...
pdf(file=paste0(output_dir, '/epi_super_PC_plot.pdf'), width=5, height=6)
ElbowPlot(super_set, ndims=50)
dev.off()

#VariableFeaturePlot(super_set, assay='integrated')

super_set <- RunUMAP(object=super_set, assay='integrated', dims = 1:10, n.neighbors=50L, min.dist=0.3,
                   spread=1, repulsion.strength=1, negative.sample.rate = 10L, n.epochs=500)
DefaultAssay(super_set) <- 'integrated'
super_set <- FindNeighbors(super_set, reduction = "pca", dims = 1:10, k.param=25, n.trees=5000)
super_set <- FindClusters(super_set, resolution=0.35, algorithm=1, n.iter=50, n.start=50)


pdf(file=paste0(output_dir, '/EpiSuper_UMAP_clusters_0.35.pdf'), width=7, height=6)
DimPlot(super_set, reduction = 'umap', label=F, group.by='seurat_clusters',
...
super_set <- AddModuleScore(super_set, features=foxm1_target_genes, name='foxm1_targets', assay='RNA')
super_set <- AddModuleScore(super_set, features=metallo_genes, name='metallothioneins', assay='RNA')
...
markers <- c('FLG','KRT17','FAM25A','RNASE7',
             'CTSV','FAM3D','DSG3','GBP6',
             'metallothioneins1','foxm1_targets1','BAK1')

markers <- c('CNN3','PDCD4','ADM')
markers <- c('metallothioneins1','FOXM1','foxm1_targets1','BAK1')

DefaultAssay(super_set) <- 'RNA'
pdf(file=paste0(output_dir, '/Epi_super_markers2_split_Dist.pdf'), width=12, height=13)
FeaturePlot(subset(super_set, location=='Distal'), markers, order=T, split.by='condition') & 
...
foxm1_plots <- lapply(c('Proximal','Distal'), function(loc) {
  lapply(c('HC','SSc'), function(cond) {
    ...
    return(FeaturePlot(subset(super_set, location==loc & condition==cond), features='foxm1_targets1', order=T) & labs(x='UMAP 1', y='UMAP 2') &
             scale_colour_gradientn(colours=c('grey80',brewer.pal(n=11,'Spectral')[6:1]), name='Avg. Gene\nExpression\n', 
...
pdf(file=paste0(output_dir, '/Epi_modulePlots.pdf'), width=12, height=4)
...
super_set <- RunUMAP(object=super_set, assay='integrated', dims = 1:10, n.neighbors=15L, min.dist=0.3,
                    spread=1, repulsion.strength=1, negative.sample.rate = 10L, n.epochs=1000)
#DimPlot(myeloid_set, group.by='Phase', label=F)


pdf(file=paste0(output_dir, '/EpiSuper_dimPlot_condition.pdf'), width=7, height=6)
DimPlot(super_set, reduction='umap', label=F, group.by='condition', shuffle=T, 
        cols=c('#FF00FF','#00FFFF','#FFFF00')) + ggtitle('')
dev.off()

save(super_set, file=paste0(output_dir, '/MP_eso_epi_super.RData'))


markers <- c('PDPN','IGFBP3','COL17A1','KRT15','DST',
             'KRT4','KRT13','SERPINB3','DSC2','IVL',
             'KRT17','KRT78','FLG','CNFN','CRCT1') 


markers <- c('KRT6A','KRT14' ,'CXCL14',
                 'IGFBP3','IGFBP6','KRT15','MKI67','PCNA')

DefaultAssay(epi_set) <- 'RNA'
pdf(file=paste0(output_dir, '/Epi_UMAP_markers.pdf'), width=16, height=9)
print(FeaturePlot(epi_set, features=markers, order=T, ncol=5) & 
        scale_colour_gradientn(colours=brewer.pal(n=9,'YlGnBu')[3:9]) & 
        theme(axis.text=element_blank(), axis.title=element_text(size=9),
              axis.ticks = element_blank()))
dev.off()
...
for (marker in markers) {
  pdf(file=paste0(output_dir, '/Epi_UMAP_',marker,'.pdf'), width=7, height=6)
  print(FeaturePlot(epi_set, features=marker, order=T) & 
          scale_colour_gradientn(colours=brewer.pal(n=9,'YlGnBu')[3:9]) & 
          theme(axis.text=element_blank(), axis.title=element_text(size=9),
                axis.ticks = element_blank()))
  dev.off()
...
pdf(file=paste0(output_dir, '/Epi_UMAP_nFeature_RNA.pdf'), width=7, height=6)
print(FeaturePlot(epi_set, features='nFeature_RNA', order=T)  & 
        scale_colour_gradientn(colours=brewer.pal(n=9,'YlGnBu')[3:9]))
dev.off()
...
epi_set <- FindNeighbors(epi_set, reduction = "pca", dims = 1:35, k.param=100, n.trees=50)
epi_set <- FindClusters(epi_set, resolution=0.25, algorithm=1, n.iter=10, n.start=10)
#epi_set <- FindClusters(epi_set, resolution=0.75, algorithm=1, n.iter=10, n.start=10)


pdf(file=paste0(output_dir, '/Epi_UMAP_clusters_k50_res0.25.pdf'), width=7, height=6)
DimPlot(epi_set, reduction = 'umap', label=T, group.by='integrated_snn_res.0.25',
...
epi_repset <- subset(epi_set, subset=integrated_snn_res.0.25 %in% c(1,4,5,6,7))
save(epi_repset, file=paste0(output_dir, '/MP_eso_epi_repCells.RData'))


Idents(epi_set) <- 'integrated_snn_res.0.25'
x <- FindMarkers(epi_set, ident.1='0', min.pct=0.1, logfc.threshold=0.2, only.pos=T)

Idents(epi_set) <- 'location'
layers_rep <- c('basal','replicating basal','replicating suprabasal','suprabasal','superficial')
prox_v_dist_HCs <- lapply(setNames(layers_rep, layers_rep), function(l) {
  df <- FindMarkers(subset(epi_set, condition=='HC' & layers_rep==l), ident.1='Proximal', min.pct=0.1, logfc.threshold=0)
  df$p_val_adj <- p.adjust(df$p_val, method='bonferroni')
  df <- df[abs(df$avg_log2FC)>0.1,]
  return(df)
  })
prox_v_dist_GERD <- lapply(setNames(layers_rep, layers_rep), function(l) {
  df <- FindMarkers(subset(epi_set, condition=='GERD' & layers_rep==l), ident.1='Proximal', min.pct=0.01, logfc.threshold=0)
  df$p_val_adj <- p.adjust(df$p_val, method='bonferroni')
  df <- df[abs(df$avg_log2FC)>0.1,]
  return(df)
  })
prox_v_dist_SSc <- lapply(setNames(layers_rep, layers_rep), function(l) {
  df <- FindMarkers(subset(epi_set, condition=='SSc' & layers_rep==l), ident.1='Proximal', min.pct=0.01, logfc.threshold=0)
  df$p_val_adj <- p.adjust(df$p_val, method='bonferroni')
  df <- df[abs(df$avg_log2FC)>0.1,]
  return(df)
  })
prox_v_dist_all <- lapply(setNames(layers_rep, layers_rep), function(l) {
  df <- FindMarkers(subset(epi_set, layers_rep==l), ident.1='Proximal', min.pct=0.01, logfc.threshold=0)
  df$p_val_adj <- p.adjust(df$p_val, method='bonferroni')
  df <- df[abs(df$avg_log2FC)>0.1,]
  return(df)
  })

save(prox_v_dist_all, prox_v_dist_HCs, prox_v_dist_GERD, prox_v_dist_SSc, file=paste0(output_dir,'/Epi_prox_v_dist_scDEGs.RData'))



for (i in 1:5) {
  print(sum(rownames(prox_v_dist_SSc[[i]]) %in% rownames(prox_v_dist_HCs[[i]]))/nrow(prox_v_dist_SSc[[i]]))
}

for (i in 1:5) {
  print(sum(rownames(prox_v_dist_GERD[[i]]) %in% rownames(prox_v_dist_HCs[[i]]))/nrow(prox_v_dist_GERD[[i]]))
}
...
pdf(file=paste0(output_dir, '/Epi_UMAP_clusters_k50_res0.25_ordered.pdf'), width=7, height=6)
DimPlot(epi_set, reduction = 'umap', label=T, group.by='seurat_clusters',
        order=as.character(c(30:0))) + ggtitle('') &
  theme(plot.title=element_text(face='bold', size=10), plot.subtitle=element_text(face='italic', size=8, hjust=0.5),
        axis.text=element_blank(), axis.title=element_text(size=8), axis.ticks = element_blank(), 
        legend.title=element_text(size=7, face='bold'), legend.text=element_text(size=6.5))
...
pdf(file=paste0(output_dir, '/Epi_clusters_markers_violin.pdf'), width=5, height=10)
print(Stacked_VlnPlot(epi_set, markers, pt.size=0, group.by='seurat_clusters', raster=F, plot_spacing=0,
                      colors=gg_color_hue(8), x_lab_rotate=T) &
...
pdf(file=paste0(output_dir, '/Epi_layers_markers_violin.pdf'), width=3, height=8)
print(Stacked_VlnPlot(epi_set, markers, pt.size=0, group.by='layers_rep', raster=F, plot_spacing=0,
                      colors=layer_cols_5, x_lab_rotate=T) &
        theme(axis.line.y = element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()))
dev.off()

epi_set$condition <- factor(epi_set$condition, levels=c('HC','GERD','SSc'))

pdf(file=paste0(output_dir, '/Epi_layers_markers_violin_cond.pdf'), width=5, height=8)
print(Stacked_VlnPlot(epi_set, markers, pt.size=0, group.by='layers_rep', raster=F, plot_spacing=0,
                      colors_use=rev(cond_cols), x_lab_rotate=T, split.by='condition') &
        theme(axis.line.y = element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()))
dev.off()


pdf(file=paste0(output_dir, '/Epi_bins_markers_violin_cond.pdf'), width=9, height=8)
print(Stacked_VlnPlot(epi_set, markers, pt.size=0, group.by='diff.bin', raster=F, plot_spacing=0,
                      colors_use=rev(cond_cols), x_lab_rotate=T, split.by='condition') &
        theme(axis.line.y = element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()))
dev.off()


module_plot(epi_set, 'Basal', c('PDPN, IGFBP3'))



library("RColorBrewer")

epi_set@meta.data$clusters_ordered <- mapvalues(epi_set@meta.data$integrated_snn_res.0.5, 
                                                from=as.character(c(0:13)),
                                                to=c('1','12','10','14','6','8','3','9','13','5','2','7','11','4'))

epi_set@meta.data$clusters_ordered <- mapvalues(epi_set@meta.data$integrated_snn_res.0.75, 
                                                from=as.character(c(0:20)),
                                                to=c('1','16','20','14','4','10','15','8','3','7','17',
                                                     '9','21','12','18','13','11','6','19','5','2'))

pdf(file=paste0(output_dir, '/Epi_UMAP_clusters_k40_res0.75_ordered.pdf'), width=7, height=6)
DimPlot(epi_set, reduction = 'umap', label=T, group.by='clusters_ordered',
        order=as.character(c(30:0))) + ggtitle('')
dev.off()
...
epi_set@meta.data$layers <- factor(mapvalues(epi_set@meta.data$clusters_ordered, from=as.character(c(1:21)),
                                             to=layers), levels=c('Basal','Suprabasal','Superficial'))

pdf(file=paste0(output_dir, '/Epi_UMAP_layers2.pdf'), width=7, height=6)
DimPlot(epi_set, reduction = 'umap', label=F, group.by='layers', 
...
epi_set@meta.data$layer_clusters <- factor(mapvalues(epi_set@meta.data$integrated_snn_res.0.2, 
                                                     from=as.character(c(0:7)),
                                                     to=layer_clusters), levels=layer_clusters)

epi_set@meta.data$compartment <- factor(mapvalues(epi_set@meta.data$integrated_snn_res.0.2, from=as.character(c(0:7)),
...
for (sample in samples) {
  c <- epi_set@meta.data[epi_set@meta.data$orig.ident==sample, cond][1]
  total <- nrow(epi_set@meta.data[epi_set$orig.ident==sample,])
  for (l in c(1:8)) {
    epi_layer_df[i,] <- c(sample, c, l, round(nrow(epi_set@meta.data[epi_set$orig.ident==sample & 
                                                                              epi_set$seurat_clusters==l,])/total,2))
    i <- i+1
  }
}
...
pdf(file=paste0(output_dir, '/Epi_layerProp_vs_condition.pdf'), width=8, height=6)
...
non_super <- subset(epi_set, layers_rep!='superficial')
...
pdf(file=paste0(output_dir, '/Epi_repLayer_vs_condition_loc.pdf'), width=4.5, height=7)
...
pdf(file=paste0(output_dir, '/Epi_layerRepProp_vs_condition.pdf'), width=7, height=4)
...
pdf(file=paste0(output_dir, '/Epi_layerRepProp_vs_condition_Region.pdf'), width=3.8, height=10)
plot_grid(p_bars, legend, rel_heights=c(1,0.1), scale=0.95,ncol=1)
dev.off()


#### Calculate cell type proportions by condition
prop_df <- data.frame(matrix(ncol=3, nrow=length(table(epi_set$seurat_clusters))))
colnames(prop_df) <- c('Cell type','Condition','Percent')
i=1
for (cond in c('HC','GERD','SSc')) {
  total <- nrow(epi_set@meta.data[epi_set$condition==cond,])
  for (type in c(unique(epi_set$seurat_clusters))) {
    print(type)
      prop_df[i,] <- c(type, cond, 
                       round(nrow(epi_set@meta.data[epi_set$condition==cond & 
                                                    epi_set$seurat_clusters==type,])/total,5))
  i <- i+1
  }
}
...
pdf(file=paste0(output_dir, '/Epi_layer_vs_condition.pdf'), width=7, height=8)
print(ggplot(prop_df, aes(x=Condition, y=Percent, fill=`Cell type`)) +
        ...
        scale_fill_manual('Layer', values=gg_color_hue(length(unique(epi_set$seurat_clusters)))))
dev.off()
...
#epi_set$basal11 <- (mean(epi_set$basal1) - epi_set$basal1)/sd(epi_set$basal1)
#epi_set$suprabasal1 <- (mean(epi_set$suprabasal1) - epi_set$suprabasal1)/sd(epi_set$suprabasal1)
#epi_set$superficial1 <- (mean(epi_set$superficial1) - epi_set$superficial1)/sd(epi_set$superficial1)

epi_set$diff.score <- epi_set$superficial1+epi_set$suprabasal1-epi_set$basal1
#epi_set$diff.score <- epi_set$suprabasal1-epi_set$basal1*2


pdf(file=paste0(output_dir, '/Epi_UMAP_diff.score.pdf'), width=7, height=7)
print(FeaturePlot(epi_set, features='diff.score', order=T) & labs(x='UMAP 1', y='UMAP 2') &
...
DefaultAssay(epi_set) <- 'RNA'
layer_km <- kmeans(FetchData(epi_set, vars=c(basal_markers, 
                                             suprabasal_markers, superficial_markers)), 3,
                   iter.max=1000, nstart=10)

#layer_km <- kmeans(epi_set@meta.data[,c('basal1', 'suprabasal1', 'superficial1')],3,
#                   iter.max=1000, nstart=10)

epi_set@meta.data$layer_cluster <- layer_km$cluster

DimPlot(epi_set, reduction = 'umap', label=T, group.by='layer_cluster',
...
pdf(file=paste0(output_dir, '/Epi_UMAP_repSubset_kmeans.pdf'), width=7, height=7)
...
library(scales)
pdf(file=paste0(output_dir, '/Epi_scatter_suprabasal_basal.pdf'), width=7, height=7)
plot(epi_set$suprabasal1, epi_set$basal1, pch=16,cex=0.25, col='navy',
...
pdf(file=paste0(output_dir, '/Epi_UMAP_superficialSubset_kmeans.pdf'), width=7, height=7)
...
pdf(file=paste0(output_dir, '/Epi_UMAP_layers_kmeans.pdf'), width=7, height=6)
DimPlot(epi_set, reduction = 'umap', label=F, group.by='layers', 
...
epi_set$layers_rep <- factor(ifelse((epi_set$layers=='basal') & (epi_set$Phase %in% c('S','G2M')),
                             'replicating basal',
                             ifelse((epi_set$layers=='suprabasal') & (epi_set$Phase %in% c('S','G2M')),
                                    'replicating suprabasal',
                                    ifelse(epi_set$layers=='basal','basal',
                                           ifelse(epi_set$layers=='suprabasal','suprabasal',
                                                  'superficial')))),
...
pdf(file=paste0(output_dir, '/Epi_UMAP_layersRep_kmeans.pdf'), width=8, height=6)
DimPlot(epi_set, reduction = 'umap', label=F, group.by='layers_rep',raster=F,
...
par(mfrow=c(4,1))
hist(epi_set$basal1, xlim=c(-4,4), breaks=40)
hist(epi_set$suprabasal1, xlim=c(-4,4), breaks=40)
hist(epi_set$superficial1, xlim=c(-4,4), breaks=40)
hist(epi_set$diff.score, xlim=c(-4,4), breaks=40)
dev.off()


pdf(file=paste0(output_dir, '/Epi_dens_layer_moduleScores.pdf'), width=6, height=8)
par(mfrow=c(3,1))
plot(density(epi_set@meta.data[epi_set$layers=='basal','basal1']), xlim=c(-2,3), ylim=c(0,3),
...
lines(density(epi_set@meta.data[epi_set$layers=='suprabasal','basal1']), col=layer_cols_3[2],lwd=3)
lines(density(epi_set@meta.data[epi_set$layers=='superficial','basal1']), col=layer_cols_3[3],lwd=3)
plot(density(epi_set@meta.data[epi_set$layers=='basal','suprabasal1']), xlim=c(-2,5), ylim=c(0,2),
     main='Suprabasal Cells',col=layer_cols_3[1],lwd=3,xlab='Module Score')
legend('topright',legend=c('Basal','Suprabasal','Superficial'), title='Module Scores', title.font=2,
       title.adj=-0.02, col=layer_cols_3,lty=1, lwd=3, cex=0.8, box.lty=0)
lines(density(epi_set@meta.data[epi_set$layers=='suprabasal','suprabasal1']), col=layer_cols_3[2],lwd=3)
lines(density(epi_set@meta.data[epi_set$layers=='superficial','suprabasal1']), col=layer_cols_3[3],lwd=3)
plot(density(epi_set@meta.data[epi_set$layers=='basal','superficial1']), xlim=c(-2,6), ylim=c(0,2.5),
     main='Superficial Cells',col=layer_cols_3[1],lwd=3,xlab='Module Score')
legend('topright',legend=c('Basal','Suprabasal','Superficial'), title='Module Scores', title.font=2,
       title.adj=-0.02, col=layer_cols_3,lty=1, lwd=3, cex=0.8, box.lty=0)
lines(density(epi_set@meta.data[epi_set$layers=='suprabasal','superficial1']), col=layer_cols_3[2],lwd=3)
lines(density(epi_set@meta.data[epi_set$layers=='superficial','superficial1']), col=layer_cols_3[3],lwd=3)
dev.off()


pdf(file=paste0(output_dir, '/Epi_dens_layer_diff.score.pdf'), width=6, height=5)
plot(density(epi_set@meta.data[epi_set$layers=='basal','diff.score']), xlim=c(-4,8), ylim=c(0,1),
...
lines(density(epi_set@meta.data[epi_set$layers=='suprabasal','diff.score']), col=layer_cols_3[2],lwd=4)
lines(density(epi_set@meta.data[epi_set$layers=='superficial','diff.score']), col=layer_cols_3[3],lwd=4)
dev.off()


##### ANALYZE COMPARTMENTS AND CELLS BY CONDITION
table(epi_set@meta.data[,c(cond,'layers')])

#epi_set$diff.col <- seq_gradient_pal(brewer.pal(n=11,'Spectral')[11:1])(rescale(epi_set$diff.score))

epi_set$diff.col <- mapvalues(epi_set@meta.data$layers, 
                                                from=c('basal','suprabasal','superficial'),
                                                to=layer_cols_3)

epi_set$diff.col5 <- mapvalues(epi_set@meta.data$layers_rep, 
                              from=c('basal','replicating basal','replicating suprabasal','suprabasal','superficial'),
                              to=layer_cols_5)

#do_BeeSwarmPlot(epi_set, 'diff.score',group.by='condition', continuous_feature = T)

swarm_dat <- do.call(rbind, lapply(split(epi_set@meta.data, epi_set@meta.data$condition),
                                   function(x) x[sample(nrow(x), 50000), 
                                                 c('condition','diff.score','diff.col5','layers_rep')]))
#swarm_dat <- epi_set@meta.data[,c('diff.score','condition','diff.col')]
swarm_dat$condition <- factor(swarm_dat$condition,levels=c('HC','GERD','SSc'))
cond_cols <- c('#8156B3','#e26b53','#A8A39d')

library(beeswarm)

pdf(file=paste0(output_dir, '/Epi_swarm_layersRep.pdf'), width=5, height=6)
par(mar=c(5,4,0,2))
...
pdf(file=paste0(output_dir, '/Epi_dens_layers_condition.pdf'), width=6, height=4)
par(mfrow=c(1,1), mar=c(5,4.5,1,2))
plot(density(epi_set@meta.data[epi_set$condition=='SSc','diff.score']), xlim=c(0,1), ylim=c(0,4.2),
...
lines(density(epi_set@meta.data[epi_set$condition=='GERD','diff.score']), col=cond_cols[2],lwd=4)
lines(density(epi_set@meta.data[epi_set$condition=='HC','diff.score']), col=cond_cols[3],lwd=4)
dev.off()

# Plot by location
library(beeswarm)
epi_set$diff.col <- mapvalues(epi_set@meta.data$layers_rep, 
                              from=c('basal','replicating basal','replicating suprabasal','suprabasal','superficial'),
                              to=layer_cols_5)

epi_subset_p <- subset(epi_set, location=='Proximal')
epi_subset_d <- subset(epi_set, location=='Distal')
...
pdf(file=paste0(output_dir, '/Epi_swarm_layersRep_loc.pdf'), width=8, height=6)
...
pdf(file=paste0(output_dir, '/Epi_dens_layers_condition_loc.pdf'), width=10, height=5)
...
pdf(file=paste0(output_dir, '/Epi_layerProp_vs_condition.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_layerProp_vs_condition_Region.pdf'), width=7, height=7)
...
pdf(file=paste0(output_dir, '/Epi_layerRepProp_vs_condition_Region.pdf'), width=7, height=7)
plot_grid(p_bars, legend, rel_widths=c(1,0.2), scale=0.95)
dev.off()
...
for (cond in c('HC','GERD','SSc')) {
  total <- nrow(epi_set@meta.data[epi_set$condition==cond,])
  for (type in c(unique(epi_set$layers))) {
    print(type)
    prop_df[i,] <- c(type, cond, 
                     round(nrow(epi_set@meta.data[epi_set$condition==cond & 
                                                    epi_set$layers==type,])/total,5))
    i <- i+1
  }
}
...
epi_hc <- subset(epi_set, subset=condition=='HC')
epi_hc <- epi_hc[, sample(colnames(epi_hc), 50000)]
epi_gerd <- subset(epi_set, subset=condition=='GERD')
epi_gerd <- epi_gerd[, sample(colnames(epi_gerd), 50000)]
epi_ssc <- subset(epi_set, subset=condition=='SSc')
...
pdf(file=paste0(output_dir, '/Epi_UMAP_diff_condition.pdf'), width=12, height=4)
...
epi_p <- subset(epi_set, location=='Proximal')
epi_d <- subset(epi_set, location=='Distal')
...
layers <- names(table(epi_set$layers_rep))
deg_list_ssc_hc <- list(); deg_list_ssc_gerd <- list(); deg_list_gerd_hc <- list()

deg_list_ssc_hc <- setNames(future_lapply(layers, function(l) {
  FindMarkers(subset(epi_set, layers_rep==l), ident.1='SSc', ident.2='HC', logfc.threshold=0, min.pct=0)}, future.seed=T), layers)
deg_list_ssc_gerd <- setNames(future_lapply(layers, function(l) {
  FindMarkers(subset(epi_set, layers_rep==l), ident.1='SSc', ident.2='GERD', logfc.threshold=0, min.pct=0)}, future.seed=T), layers)
deg_list_gerd_hc <- setNames(future_lapply(layers, function(l) {
  FindMarkers(subset(epi_set, layers_rep==l), ident.1='GERD', ident.2='HC', logfc.threshold=0, min.pct=0)}, future.seed=T), layers)

# equal size bins by diff.score

epi_set$diff.bin <- cut(epi_set$diff.score, 
                        breaks=unique(quantile(epi_set$diff.score,
                                               probs=seq.int(0,1,by=1/10))), include.lowest=T)

bins <- names(table(epi_set$diff.bin))
...
deg_list_ssc_hc_bins <- setNames(future_lapply(bins, function(l) {
  FindMarkers(subset(epi_set, diff.bin==l), ident.1='SSc', ident.2='HC', logfc.threshold=0.2, min.pct=0.1)}, future.seed=T), bins)
deg_list_ssc_gerd_bins <- setNames(future_lapply(bins, function(l) {
  FindMarkers(subset(epi_set, diff.bin==l), ident.1='SSc', ident.2='GERD', logfc.threshold=0.2, min.pct=0.1)}, future.seed=T), bins)
deg_list_gerd_hc_bins <- setNames(future_lapply(bins, function(l) {
  FindMarkers(subset(epi_set, diff.bin==l), ident.1='GERD', ident.2='HC', logfc.threshold=0.2, min.pct=0.1)}, future.seed=T), bins)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_fc',fc_min,'.pdf'), width=8, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_SSc_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_GERD_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_SScGerd_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_conds_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_loc_fc',fc_min,'.pdf'), width=8, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_loc_SSc_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_loc_GERD_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_loc_SScGerd_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_degVenn_layers_conds_loc_fc',fc_min,'.pdf'), width=6, height=4)
...
pdf(file=paste0(output_dir, '/Epi_diffBins_nDEGs.pdf'), width=9, height=4)
...
pdf(file=paste0(output_dir, '/Epi_UMAP_diffBins_80-90.pdf'), width=6, height=5)
DimPlot(epi_set, group.by='diff.bin', cols=c(rep('khaki',8),'navy','khaki'))
dev.off()
...
Idents(epi_set) <- 'condition'
FeatureScatter(epi_set, feature1='diff.score',feature2='S100A9', cols=cond_cols)

epi_set$diff.bin <- cut(epi_set$diff.score, 
                        breaks=unique(quantile(epi_set$diff.score,
                                               probs=seq.int(0,1,by=1/10))), include.lowest=T)

pdf(file=paste0(output_dir, '/Epi_vlnPlot_cond_diffbin_s100A9.pdf'), width=9, height=4)
VlnPlot(epi_set, features=c('S100A8','S100A9'),split.by='condition',group.by='diff.bin', cols=cond_cols,
        stack=T)
dev.off()

FeatureScatter(epi_set, feature1='diff.score',feature2='S100A9', shuffle=T, cols=cond_cols, smooth=T)
...
pdf(file=paste0(output_dir, '/Epi_diffBins_pctDEGs_loc_fc0.25.pdf'), width=9, height=9)
...
epi_bulk <- AggregateExpression(epi_set, assays='')
...
for (loc in c('Proximal', 'Distal')) {
  ...
  sc_obj <- subset(epi_set, location==loc)
...
diff_score_all <- cbind(cell=rownames(epi_set@meta.data), differentation.score=epi_set$diff.score)

cells <- 1:ncol(epi_set)
...
diff_scores <- as.data.frame(cbind(cell=rownames(epi_set@meta.data), pseudotime=epi_set$diff.score))
...
expv <- GetAssayData(epi_set, slot='counts')
...
##### MONOCLE ANALYSIS
epi_cds <- as.cell_data_set(epi_set)
...
epi_cds@clusters@listData[['UMAP']][['clusters']] <- epi_set@active.ident

epi_cds@int_colData@listData[['reducedDims']]@listData[['UMAP']] <- epi_set@reductions$umap@cell.embeddings
...
jpeg(file=paste0(output_dir, '/Epi_monocleTrajectory.jpg'), width=7, height=8, units='in',res=150)
